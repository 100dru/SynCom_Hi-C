...
p1 = scores_sensspec_sample %>% 
  ggplot(aes(x = filt_type, y = sample)) + 
  geom_tile(aes(fill = count), color = "black") +
  scale_fill_distiller(palette = "Greys", direction = 1, na.value = NA) + 
  geom_label(aes(label = count), alpha = 0.3) +
  coord_equal() +
  theme_classic() +
  theme(legend.position = "bottom")

p2 = scores_sensspec_experiment %>% 
  ggplot(aes(x = filt_type, y = experiment)) + 
  geom_tile(aes(fill = count), color = "black") +
  geom_label(aes(label = count), alpha = 0.3) +
  scale_fill_distiller(palette = "Greys", direction = 1, na.value = NA) + 
  coord_equal() +
...
p12 = p1 + p2 
p12
p3 = scores_sensspec_sample_experiment %>% 
  gather(stt, val, min:sum) %>% 
  mutate(stt = factor(stt, levels = unique(stt))) %>% 
  ggplot(aes(x = filt_type, y = experiment)) + 
  geom_tile(aes(fill = val), color = "black") +
  geom_label(aes(label = round(val, digits = 1)), alpha = 0.3) +
  scale_fill_distiller("count", palette = "Greys", direction = 1, na.value = NA) + 
  facet_grid(~ stt) +
  coord_equal() +
  theme_classic() +
  theme(legend.position = "bottom")

p12 / p3 + plot_layout(heights = c(2, 1))



p4 = scores_sensspec_host %>% 
  ggplot(aes(x = filt_type, y = Host)) +
...
p5 = scores_sensspec_hostexperiment %>% 
  ggplot(aes(x = filt_type, y = Host)) +
  geom_tile(aes(fill = count)) +
  geom_label(aes(label = count), alpha = 0.3) +
  scale_fill_distiller(palette = "Greys", direction = 1, na.value = NA) + 
...
p6 = scores_sensspec_phage %>% 
  ggplot(aes(x = filt_type, y = Phage)) +
  geom_tile(aes(fill = count)) +
  geom_label(aes(label = count), alpha = 0.3) +
  scale_fill_distiller(palette = "Greys", direction = 1, na.value = NA) + 
  coord_equal() +
...
p7 = scores_sensspec_phageexperiment %>% 
  ggplot(aes(x = filt_type, y = Phage)) +
  geom_tile(aes(fill = count)) +
  geom_label(aes(label = count), alpha = 0.3) +
  scale_fill_distiller(palette = "Greys", direction = 1, na.value = NA) + 
...
(p4 + p5) / (p6 + p7) + plot_layout(heights = c(1, 2))
